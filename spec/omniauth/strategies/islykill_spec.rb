...
def post_xml(xml=:example_response)
  post "/auth/islykill/callback", {'SAMLResponse' => load_xml(xml)}
end

describe OmniAuth::Strategies::Islykill, :type => :strategy do
  include OmniAuth::Test::StrategyTestCase

  let(:auth_hash){ last_request.env['omniauth.auth'] }
  let(:saml_options) do
    {
      :assertion_consumer_service_url     => "http://localhost:3000/auth/islykill/callback",
  ...
  let(:strategy) { [OmniAuth::Strategies::Islykill, saml_options] }
  ...
  describe 'POST /auth/islykill/callback' do
  ...
  describe 'GET /auth/islykill/metadata' do
    before do
      get '/auth/islykill/metadata'
    end
